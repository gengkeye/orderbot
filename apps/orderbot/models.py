...
class TeleUser(models.Model):
	...
	def can_notice(self):
		...
		if not self.last_notice_at:
			return True
		if self.last_notice_at + datetime.timedelta(days=time_interval_days) <= datetime.datetime.today().date():
			return True
		return False
...
class TeleMembership(models.Model):
	...
	def __str__(self):
		return "%s %s: %s"%(self.user.name, self.group.name, str(self.balance))
...
class TeleOrder(models.Model):
	...
	def discard(self):
		...
		for item in items:
			...
			try:
				membership, r = TeleMembership.objects.get_or_create(user=item_user, group=self.group)
				before_balance = membership.balance
				membership.balance = before_balance + item['total_price']
				membership.save()
				self.balances.create(
					user=item_user,
					group=self.group,
					before=before_balance,
					after=membership.balance,
					diff=item['total_price'],
...
class TeleBalanceHistory(models.Model):
	...
	remarks = models.CharField(max_length=100, blank=True)
	created_by = models.CharField(choices=CREATOR_CHOICES, max_length=10)
	date_created = models.DateTimeField(auto_now_add=True)
